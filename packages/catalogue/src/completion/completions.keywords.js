...
import { rangesEqual, toVsRange } from "@creatureslsp/extension-util";
import { createCompletionItem } from "../completions.js";
const arrayOrTag = ["ARRAY", "TAG"];
export function appendKeywordCompletions(cursorData, parserItems, out) {
    ...
    for (const keyword of keywords) {
        const item = createCompletionItem(keyword, range);
        out.push(item);
    }
}
export function getKeywordsForCompletions(cursorData, parserItems) {
...
function getItemBefore(items, target) {
    let before = null;
...
function getItemAfter(items, target) {
    const targetRange = target.textRange;
