...
import { BINARY_PARSER_TYPE, BRACKET_STRING_PARSER_TYPE, BYTE_STRING_PARSER_TYPE, CAOS2_COMMENT_TYPE, CHAR_PARSER_TYPE, COMMAND_TOKEN_PARSER_TYPE, COMMENT_TYPE, EQ_JOIN_PARSER_TYPE, EQ_OP_PARSER_TYPE, ERROR_PARSER_TYPE, FLOAT_PARSER_TYPE, INDEXED_VAR_PARSER_TYPE, INT_PARSER_TYPE, PICT_DIMENSION_PARSER_TYPE, PLACEHOLDER_TYPE, QUOTE_STRING_PARSER_TYPE, TOKEN_PARSER_TYPE } from "./constants.js";
 ...
 * @param listener
 */
export function walkCaosParseResult(result, listener) {
    ...
    for (let item of result.items) {
        switch (item.typeToken) {
            ...
            case COMMAND_TOKEN_PARSER_TYPE:
                listener.onCommandToken(item);
                continue;
            case INDEXED_VAR_PARSER_TYPE:
                listener.onIndexedVar(item);
                continue;
 ...
 */
export class CaosContextListenerBase {
    /**
    ...
    onPictDimension(_token) {
    }
    onToken(_token) {
    }
    onPlaceholderText(_token) {
    }
    onComment(_token) {
    }
    onCaos2Comment(_token) {
    }
}
