...
import { arrayUnique, filterNotNull, getExtension, getFileName, getFileNameWithoutExtensions, stripSurroundingQuotes } from "@creatureslsp/extension-util";
...
export async function getC3DSTagValueCompletions(thisFileName, tag, command, directory, commands, getFiles) {
    if (command) {
        return getCommandValueCompletions(thisFileName, command.toUpperCase(), directory, commands, getFiles);
    }
    if (tag) {
 ...
 * @param getFiles
 */
async function getCommandValueCompletions(thisFileName, command, directory, previousCommands, getFiles) {
    ...
    let previous = getCommandValues(previousCommands, command)
        .map((s) => s.toLowerCase());
    const out = [];
 ...
 */
async function getTagValueCompletions(tag, commands, getFiles) {
...
export async function getGeneticsFile(additionalFiles, getFiles) {
 ...
 */
async function getSpriteTagValueCompletions(tag, inlinedOrDepended, getFiles) {
    ...
    let fileNames = await getAllFileNames(["s16", "c16"], inlinedOrDepended, getFiles);
    let out = inlinedOrDepended
        .filter(fileName => {
...
async function getAllFileNames(extensions, inlinedOrDepended, getFiles) {
