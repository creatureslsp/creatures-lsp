...
export function getCaos2CommandCompletions(variant, allComments, token, range) {
...
function getC1CommandCompletions(commandsSoFar, token) {
...
function getC2CommandCompletions(commandsSoFar, token) {
    const commandsSoFarLower = commandsSoFar.map(c => c.toLowerCase());
    const out = [];
    if (commandsSoFarLower.indexOf("cob-file") < 0) {
        out.push("Cob-File");
...
function getC3DSCommandCompletions(commandsSoFar, token) {
    const commandsSoFarLower = commandsSoFar.map(c => c.toLowerCase());
