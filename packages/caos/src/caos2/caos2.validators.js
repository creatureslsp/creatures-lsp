import { getArrayAccessFileNameIndex, PLUS_OR_MINUS_WITH_NUMBER_REGEX, getExtension, hasExtension, stripSurroundingQuotes, extname } from "@creatureslsp/extension-util";
...
function getFilenameWithoutArrayAccess(path) {
    if (path.indexOf('[') < 0) {
        return path;
    }
...
function isBasicSpriteOrBlk(value) {
    return isSprite(value, true);
}
function isBasicSprite(value) {
    return isSprite(value, false);
}
function isSprite(value, canBeBlk) {
