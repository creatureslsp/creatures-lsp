...
import { getSemanticTokensLegend } from "@creatureslsp/caos";
...
import { formatUriForRead, getBool, isVsCode, setLspRunner, trimFileSchemePrefix } from "@creatureslsp/extension-util";
...
connection.onInitialize((params) => {
    ...
    clientCapabilities.hasConfigurationCapability = (!!capabilities.workspace && !!capabilities.workspace.configuration);
    clientCapabilities.hasSemanticTokensCapabilities = (!!capabilities.workspace && !!capabilities.workspace.semanticTokens);
    clientCapabilities.hasWorkspaceFolderCapability = (!!capabilities.workspace && !!capabilities.workspace.workspaceFolders);
    clientCapabilities.hasDiagnosticRelatedInformationCapability = (!!capabilities.textDocument &&
        !!capabilities.textDocument.publishDiagnostics &&
        !!capabilities.textDocument.publishDiagnostics.relatedInformation);
    clientCapabilities.hasGotoDefinitionCapabilities = (!!capabilities.textDocument &&
        !!capabilities.textDocument.documentLink);
    clientCapabilities.hasFormattingCapabilities = (!!capabilities.textDocument &&
        !!capabilities.textDocument.formatting);
    clientCapabilities.hasInlayHintsCapabilities = !isVsCode() && (!!capabilities.textDocument &&
        !!capabilities.textDocument.inlayHint);
    let noCompletions = getBool(params.initializationOptions?.noCompletions);
    clientCapabilities.hasCompletionCapabilities = !noCompletions && (!!capabilities.textDocument &&
        !!capabilities.textDocument.completion);
    clientCapabilities.hasHoverCapabilities = (!!capabilities.textDocument &&
        !!capabilities.textDocument.hover);
    clientCapabilities.hasSymbolsCapabilities = (!!capabilities.textDocument &&
        !!capabilities.textDocument.documentSymbol);
    clientCapabilities.hasCompletionCapabilities = (!!capabilities.textDocument &&
        !!capabilities.workspace?.configuration);
    clientCapabilities.hasWatchFilesCapabilities = (!!capabilities.workspace || !!capabilities.workspace.didChangeWatchedFiles);
    clientCapabilities.hasReferencesCapability = (!!capabilities.textDocument && !!capabilities.textDocument.references);
    ...
    clientCapabilities.hasFileRenameCapabilities = useRename && (!!capabilities.textDocument && !!capabilities.textDocument.rename?.prepareSupport);
    clientCapabilities.hasSymbolRenameCapabilities = useRename && (!!capabilities.textDocument && !!capabilities.textDocument.rename);
    const glob = "**/*.{" + [
        "cos",
        "catalogue",
        "ps", // PRAY
