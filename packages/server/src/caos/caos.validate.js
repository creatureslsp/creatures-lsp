import { ERROR_TYPE_TYPE_ERROR, hasSubroutine, } from "@creatureslsp/caos";
import { parseCaos, parseCaosWithin, } from "@creatureslsp/caos/parser";
import { validateCaos } from "@creatureslsp/caos/validation";
import { offsetRange, sortTextRanges, toVsRange } from "@creatureslsp/extension-util";
...
const validationSignals = new Map();
const continueWhileCallback = (documentURI, version) => {
 ...
 * @param range
 */
export function getCaosDocumentValidationErrors(document, documentVersion, range) {
    const { settings, text } = document;
    const errors = validateWithConstraints(document.documentUri, documentVersion, settings.variant, text, settings.strictSpaces ?? false, settings.maxNumberOfProblems, range);
    const workspaceUri = getWorkspaceUriForFile(document.documentUri);
    const inspectionErrors = collectCaosInspectionErrors(workspaceUri, document.documentUri, documentVersion, settings.variant, text, range);
 ...
 * @param range
 */
function validateWithConstraints(documentURI, index, variant, code, strictSpaces, maxNumberOfProblems = Number.MAX_VALUE, range) {
    let errors = [];
    try {
...
function abortOldRequests(documentURI, newVersion) {
...
function validateParserResult(documentURI, index, variant, text, strictSpaces, range) {
    if (text == null) {
        return [];
    }
    if (text.length < 1) {
    ...
    if (range != null) {
        ...
        signal.throwIfAborted();
        parseResult = parseCaosWithin(variant, text, range.start.line, range.start.character, range.end.line, range.end.character);
    }
    else {
    ...
    const errors = parseResult
        ...
        .concat(...validateCaos(parseResult.variant, parseResult, () => { signal.throwIfAborted(); }));
    let errorMarkers = errors
        .filter(e => {
...
function getSpacingError(last, item) {
