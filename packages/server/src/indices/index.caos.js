...
export async function indexCaosFile(workspaceUri, documentOrURI, range) {
    if (isIndexingPaused()) {
        return false;
    }
...
function index(workspaceUri, documentUri, parseResult, range) {
    if (isIndexingPaused()) {
        return;
    }
    ...
    const workspaceUriString = workspaceUri ?? getWorkspaceUriForFile(documentUri);
...
export function clearCaosDocumentIndices(workspaceUri, documentUri, range) {
    if (isIndexingPaused()) {
        return;
    }
