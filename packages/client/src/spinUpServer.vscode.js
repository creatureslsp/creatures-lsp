...
export async function spinUpServer(context, document, defaultClient, clientOptions) {
    ...
    const serverModule = context.asAbsolutePath(path.join('packages', 'server', 'dist', 'node', 'server.cjs'));
    ...
    if (typeof uri !== "string" && uri.scheme === 'untitled') {
        ...
        defaultClient = new LanguageClient('caos-language-server', 'CAOS Language Server', serverOptions, untitledFileClientOptions);
    ...
    putClient(folderURI, null);
    // Initialize options with a specific workspace folder
    const clientWithFolderOptions = { ...clientOptions, workspaceFolder: folder };
    // If the extension is launched in debug mode then the debug server options are used
    // Otherwise the run options are used
    // noinspection SpellCheckingInspection
    ...
    const client = new LanguageClient('caos-language-server', 'CAOS Language Server', serverOptions, clientWithFolderOptions);
