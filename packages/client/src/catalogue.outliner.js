...
export class CatalogueSymbolProvider {
    /**
...
function toVsCodeSymbol(entry) {
    ...
    const vsSymbol = new DocumentSymbol(keyword + " " + (entry.catalogueNameParserItem?.value ?? ""), "", SymbolKind.Enum, toVsCodeRange(range), toVsCodeRange(headerRange));
    vsSymbol.children = []; //entry.valueParserItems.map(mapChild);
    return vsSymbol;
}
function mapChild(parserItem, i) {
    return new DocumentSymbol(i + ": " + parserItem.value + "", "", SymbolKind.String, toVsCodeRange(parserItem.textRange), toVsCodeRange(parserItem.textRange));
}
